...
| ② Title | 30px | -0.15px | About 일반 헤딩·인용문(`.insight`)·키 사진 문구. **문구형 서브타이틀(`.btd-middle__desc` 3곳: 통계 위·갤러리 위·선물 위)과 마무리 문구(`.cv-outro`)는 이 표 밖의 예외로 27px / 500**(요청값 — 30→28→30→25→26→27로 바뀌어 온 끝값). `.btd-middle__title`은 통합 페이지에선 안 쓰임(독립 페이지용) |
...
- **폰트 5단계 26 / 22 / 18 / 15 / 13**: ① 26 = About 오프닝·롤링 헤딩·캡션·섹션 히어로 타이틀 ② 22 = About 일반 헤딩·인용문·키 사진 문구·서브타이틀·FAQ 타이틀 ③ 18 = 통계 라벨·여정 번호·선물 제목·CV 행 제목 ④ 15 = 본문·헤더 메뉴/Apply·버튼·FAQ ⑤ 13 = 크레딧·카드 부제·저자명. 예외: 통계 숫자 34/36px, 모달 닫기 ×, 히어로 소개 문구 18px(데스크톱 21px에 대응). 웨이트·색은 데스크톱과 동일
...
순서: 오프닝 헤딩(`--intro`, 38px/600) → **히어로 사진 롤링**(`about-hero-roll.js`: pin된 채 사진 5장이 넘어가고 4번째에 캡션, 5번째가 풀스크린으로 커지며 디졸브) → 롤링 직후 헤딩(`--blur-in`) → **스크랩북**(사진 2열, 오른쪽 열 155px 오프셋) → 배지 앞 헤딩 → SC 배지(GIF) → 인용문 1 → **키 사진 프레임**(중앙 정지 → 문구 1 → 풀스크린 확대 + 문구 2 → 디졸브) → 마지막 인용문 → **트로피(3D)**. 인용문(`.insight`)은 전부 30px/500. 키 사진은 `assets/01-about/about-key-photo.jpg`(실제 사진으로 교체됨, 이전 임시 회색 배경+`key-photo.png`은 폐기).
...
- **Special Gift**: 2열 **카드 그리드**(예전 토글/아코디언 목록에서 재설계됨). 카드 = 위 구분선 + 제목 + "더보기"(→ `BeyondTheDoor-gift1~4.html`, 사진 그리드 상세 페이지) + 겹친 썸네일 3장(평소엔 **기울기 없이 똑바로** 앞 사진 뒤에 나란히 겹쳐 있고, **`.btd-gift__stack`에 마우스를 올리면 양옆 사진이 ±118px 밀려나며 ±7° 기울어지며 펼쳐짐**, 0.4s ease — 브랜디자인 clients 페이지 참고. 모바일은 ±88px). 선물 4종: 스페셜 웰컴 굿즈 / 브랜드 콜라보 굿즈 / 프리미엄 가구 협찬 / 페어웰 기프트
  - **크레딧이 있는 건 gift2(브랜드 콜라보 굿즈)뿐** — gift1/3/4의 `.gift-detail__grid`는 아직 사진만 있고 크레딧 없음. gift2는 `.gift-detail__item` 안에 `<p class="gift-detail__credit">Photo by. 이름</p>`(사진 우측 하단, 13px/400, `#dddddd`, 그림자 없음 — `.about-photo__credit`/`.btd-journey__rolling-credit`와 같은 톤)를 추가해 32장 전부 채움. 같은 패턴을 gift1/3/4에도 쓸 것
  - **gift2 사진 32장의 출처**: `assets/03. program /gift /2. 브랜드 콜라보 굿즈 /`(iCloud 안의 원본 raw 사진 폴더, 공백·한글 포함이라 git 미추적)를 사용자가 직접 `N-이름.확장자`(예: `8-리루홈.jpg`)로 정리해 둔 뒤 그대로 소스로 씀 — 원본 중 출처 불명(크레딧 없는) 사진은 사용자가 그 폴더에서 미리 삭제함. `N`은 그대로 `gift2-NN`(2자리, 01~32) 번호가 되고 `이름`은 그대로 크레딧 문구("Photo by. 이름")가 됨(사람 이름이 없는 순수 제품컷 1장은 삭제되어 지금은 없음). 원본은 2560×3400급 풀 해상도라 `sips -Z 1000 -s format jpeg -s formatOptions 80`로 리사이즈해서 `assets/03-program/gift/gift2/`에 저장(총 5.4MB, avif 원본 1장은 sips가 못 열어서 리사이즈 없이 그대로 복사 — 이미 174KB라 문제 없음). **이 raw 폴더는 이제 다 소진됨**(더 이상 남은 미사용 사진 없음) — 다른 gift 항목에 같은 작업을 할 땐 이 명명 규칙(`N-이름.ext` → `Photo by. 이름`)을 그대로 따를 것
  - **FindTheKey.html 카드의 부채꼴 썸네일 3장**(`.btd-gift__stack-photo--left/right/front`)도 각 gift 폴더의 사진을 직접 가리킴 — gift2는 `gift2-04/05/06.jpg`(front=04) 사용 중, 바꾸려면 이 세 `<div>`의 `background-image`만 고치면 됨
