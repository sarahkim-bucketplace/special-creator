...
- 카드 클릭 → **팝업**(`ou-modal.js`가 `OpportunitiesUnlocked-0N.html`을 fetch해서 `.detail`+스타일을 주입). 상세 사진은 케이스별 폴더(`assets/02-opportunities/01-…~05-…`, `thumb.*` + 상세 이미지). 텍스트 라벨이 있는 사진은 `--contain` 클래스(02번 슬라이드 3). 이전/다음 링크에는 사례 제목이 들어감(05는 다음 없음). **데스크톱**의 닫기 버튼(`.ou-modal__close`, Figma 252:755)은 배경 없는 흰 X, 카드를 감싸는 `.ou-modal__frame`(카드 크기에 맞춰 hug) 바깥쪽에 절대 위치 — 카드 위가 아니라 카드 **옆** 우측 상단
  - **모바일(≤900px)만 완전히 다른 상세 레이아웃** (newmixcoffee.com/ko 프로젝트 페이지 참고, 직접 요청): 데스크톱의 "한 프레임 안에서 좌우로 넘기는" 사진 캐러셀 + X 닫기 대신, **뒤로가기 화살표 → 텍스트 → 사진 전부 세로로 쭉 이어짐**(더 이상 드래그/화살표로 넘길 필요 없음) 순서로 바뀜. 구분선(`.detail__text`의 `border-top`, `.detail__nav-line`)도 모바일에선 전부 제거. `OpportunitiesUnlocked-01~05.html`은 전부 `.detail`의 첫 자식으로 `.detail__back-btn`(Figma 노드 256:756, `assets/icon-back.svg`)을 갖고 있고(기본은 `display:none`, ≤900px에서만 보임), CSS는 `order`로 세 블록(뒤로가기/`.detail__content`/`.detail__photo`) 순서를 재배치, `.detail__photo-track`을 `display:contents`로 만들어 슬라이드들이 `.detail__photo`의 flex column에 직접 참여하게 함(각 사진 `width:100%; height:auto`, 더 이상 절대 위치/opacity 토글 없음). **이 `.detail` 마크업은 독립 페이지와 모달 팝업 둘 다에 동일하게 쓰이므로**, 뒤로가기 버튼 클릭 처리(`detail-photo-carousel.js`의 `initPhotoCarousel`)가 컨텍스트를 구분함 — 모달 안(`root !== document`)이면 `#ouModalClose`를 클릭해 팝업을 닫고, 독립 페이지면 진짜 `history.back()`(히스토리 없으면 OU 섹션 링크로 대체). `FindTheKey.css`는 모바일에서 `.ou-modal__close`(X)를 숨기고, 독립 페이지용 고정 헤더(105px) 여백(`.detail`의 `padding-top`)을 모달 안에서만 0으로 취소함(모달엔 헤더가 없어서)
