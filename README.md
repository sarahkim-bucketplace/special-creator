...
폰트·색·간격을 이번에 일괄 정리했음. 새 요소를 추가할 때 임의 값을 쓰지 말고 아래 값만 쓸 것. **모바일(≤900px, ≤600px) 값은 아직 정리 안 함** — 데스크톱 기준.
...
- **모바일(≤900/600px)**: 폰트·간격을 5단계 규칙으로 아직 정리하지 않음. 새로 생긴 scroll-snap/멈춤 스크립트가 모바일에서 어떻게 동작하는지도 미확인
