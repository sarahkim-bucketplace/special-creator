...
- **폰트 5단계 26 / 22 / 20 / 14 / 13**(②③④는 2026-09 직접 요청으로 재조정 — 예전 22/18/15였음): ① 26 = 섹션 히어로 타이틀(`.hero__title`, OU/BTD/CV 공용, 안 바뀜) ② 22, **볼드(700, 사이트 "700 안 씀" 규칙의 명시적 예외)** = 타이틀(`.cv-outro`/`.btd-middle__title`/`.faq__title`) ③ 20, **볼드(700)** = 서브타이틀(통계 라벨·`.btd-gift__title`·`.cv-row__title-main`·여정 번호·`.btd-middle__desc`) ④ 14, 웨이트는 그대로 = 본문(여정 설명·CV 본문·CV 지원 버튼·FAQ 질문답변·OU 카드 제목·선물 더보기·헤더 메뉴/Apply) ⑤ 13(안 바뀜) = 크레딧·카드 부제·저자명. **트로피 힌트(`.trophy-hint`, 15px)는 About 소속이라 이 재조정 대상이 아니고 그대로임**. 예외: 통계 숫자 34/36px(⚠️ 다른 항목과 다르게 아직 900px이 아니라 600px 기준 — 다음에 통일 예정), 모달 닫기 ×, 히어로 소개 문구 18px(데스크톱 21px에 대응). 색은 데스크톱과 동일
