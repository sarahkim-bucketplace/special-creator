...
- **서브타이틀 → 콘텐츠 130px**: `.btd-middle` 블록(직접 만나는 시간 갤러리 위, 함께한 시간 선물 위), FAQ. **`.btd-middle`의 타이틀은 지웠고 본문 한 줄이 서브타이틀** — 세 블록(통계 위·갤러리 위·선물 위) 모두 **가운데 정렬 30px/500**(서브타이틀 크기로 통일 — 처음 28px로 했다가 About 헤딩·FAQ 제목(30px)과 맞추려고 30px로 정리). 통계 위 블록만 문구→숫자 **150px**. 선물 카드만 예외로 카드 안쪽 rule이 문구 아래 110px에 오게 함(`.btd-gift { margin-top: 62px }` = 130 - 카드 padding 48 - 사용자 요청 20)
