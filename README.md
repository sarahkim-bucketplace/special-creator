...
1. **여정 리스트** (`.btd-journey`, Figma `28:1004`): 번호 붙은 단계 5개(오프닝 밋업 → 브랜드 콜라보 → 스페셜 크리에이터 활동 → 오프라인 밋업 → 페어웰), 각 단계마다 위에 구분선 + 왼쪽 텍스트(번호+제목, 설명) + 오른쪽 사진 2장(390×252). 단계 사이에 아래방향 화살표(`assets/icon-go.svg`를 회전 없이 그대로 씀 — 원래 오른쪽 화살표로 쓸 때만 `-90deg` 돌렸던 거라 아래쪽 화살표엔 회전이 필요 없음)
   ...
   - **사진 (`assets/Beyond-the-Door/journey/`, 폴더명 소문자 `journey` — 대문자 `Journey`로 잘못 참조했다가 고침. macOS는 대소문자 구분을 안 해서 로컬에선 안 걸렸지만 대소문자 구분하는 배포 환경(GitHub Pages 등)에선 404 났을 것)**: 현재 2/3/5단계만 채워짐(`02-01.png`/`02-02.jpg`, `03-01.jpg`/`03-02.jpg`, `05-1.jpg`/`05-2.jpg`), 1/4단계는 아직 회색 박스. 파일명 규칙이 살짝 다름(`05-1` vs `02-01`) — 그대로 씀
   - **hover 시 원본 이미지 플로팅**: `.btd-journey__photo` 박스가 가로형(390:252)인데 실제 사진은 대부분 세로형이라 `object-fit: cover`로 심하게 잘림. 그래서 항상 보이는 썸네일은 `.btd-journey__photo`의 `background-image`(잘린 채로 표시)로, hover 시에만 나타나는 원본 비율 미리보기는 별도 요소 `.btd-journey__photo-float`(평소엔 `opacity:0`, hover 시 `opacity:1` + `scale(0.95)→1`, 박스 중앙에 `position:absolute` + `transform:translate(-50%,-50%)`로 뜸)로 분리해서 구현함. **같은 `<img>` 하나로 두 가지를 다 하려고 하면 hover로 이동시키는 순간 원래 자리의 썸네일이 통째로 사라지는 버그가 남**(실제로 한 번 겪음) — 반드시 두 개의 별도 레이어(배경 썸네일 + 플로팅용 img)로 나눠야 함
...
3. **사진 모자이크 갤러리** (`.btd-gallery`, Figma photo-01~08 + 무명 노드 하나): Figma 절대좌표를 분석해보면 사실 꽤 깔끔한 구조였음 — 1행은 박스 2개(41% + 나머지), 2행은 동일 너비 3열이고 각 열이 내부적으로 박스 2개를 세로로 쌓은 것(가운데 열만 아래쪽이 작은 박스 2개가 가로로 나란한 형태). 세 열의 내부 합산 높이가 전부 580px로 딱 맞아떨어져서, 이 3열 구조로 확신하고 구현함. 실제 렌더링은 `aspect-ratio`로 Figma 비율만 맞추고, 폭은 flex로 반응형 처리(절대 px 좌표 그대로 베끼지 않음)
   ...
   - **사진 9장 전부 채워짐** (`assets/Beyond-the-Door/meetup/image-1~9.jpg`, DOM 순서대로 1~9번 매칭 — 배경 이미지로 넣음, `background-size:cover`). 여정 리스트 사진과 달리 **hover 확대(원본 플로팅) 기능은 안 넣음** — 한 번 갤러리에도 넣어봤다가 "어색하다"는 피드백으로 뺌. 갤러리 박스 비율이 실제 사진과 대체로 잘 맞아서(여정 리스트처럼 가로형 박스에 세로형 사진이 심하게 잘리는 문제가 없음) 여정 리스트만큼 필요하지 않았던 것으로 보임. hover-float 관련 CSS 클래스는 `.btd-photo-float`(원래 `.btd-journey__photo-float`였다가 갤러리에도 재사용해보려고 일반화한 이름)이고, 현재는 `.btd-journey__photo:hover .btd-photo-float` 규칙만 남아있어 여정 리스트에서만 동작함
